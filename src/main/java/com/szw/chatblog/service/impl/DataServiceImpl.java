...
public class DataServiceImpl implements DataService {
    ...
    public void recordIp(String ip) {
        // 拼接日期
        String key = RedisConstant.redisDailyUvKey(dateFormat.format(new Date()));
        // 加入HyperLogLog
    ...
    public Long calIntervalUv(Date start, Date end) {
        ...
        while (!calendar.getTime().after(end)) {
            String key = RedisConstant.redisDailyUvKey(dateFormat.format(calendar.getTime()));
            keys.add(key);
            calendar.add(Calendar.DATE, 1);
        }

        String key = RedisConstant.redisIntervalUvKey(dateFormat.format(start), dateFormat.format(end));
