...
import com.szw.chatblog.dto.UserDto;
import com.szw.chatblog.dto.UserDtoForLogin;
import com.szw.chatblog.mapper.UserMapper;
import com.szw.chatblog.pojo.User;
import com.szw.chatblog.service.UserService;
import com.szw.chatblog.utils.AliSendSms;
import com.szw.chatblog.utils.FormatChecker;
import com.szw.chatblog.utils.JwtUtil;

import com.szw.chatblog.constant.RedisConstant;
import com.szw.chatblog.constant.SystemConstants;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService, UserDetailsService {
    ...
    public Map<String, Object> register(String phone, String code, String password) {
        ...
        String codeStored = stringRedisTemplate.opsForValue().get(RedisConstant.USER_CODE_KEY + phone);
    ...
    public void saveUserJwt2Redis(User user, String token) {
        stringRedisTemplate.opsForValue().set(RedisConstant.USER_TOKEN_KEY + user.getUserId(), token, RedisConstant.USER_TOKEN_TTL, TimeUnit.MINUTES);
    }
    ...
    public String sendPhoneCaptcha(String phone) {
        // 生成并发送验证码
        String code = "";
        code = sendSms.sendSms(phone);
        log.info("The code is {}.", code);
        // code存入Redis
        stringRedisTemplate.opsForValue().set(RedisConstant.USER_CODE_KEY + phone, code, RedisConstant.USER_CODE_TTL, TimeUnit.MINUTES);
        return code;
    }
    ...
    public void saveUser2Redis(User user) {
        ...
        stringRedisTemplate.opsForHash().putAll(RedisConstant.USER_INFO_KEY + user.getUserId().toString(), userMap);
        stringRedisTemplate.expire( RedisConstant.USER_INFO_KEY + user.getUserId().toString(), RedisConstant.USER_INFO_TTL, TimeUnit.MINUTES);
    }
    ...
    public void logout() {
        ...
        // 删除缓存中的Token
        stringRedisTemplate.delete(RedisConstant.USER_TOKEN_KEY + userId);
    }
