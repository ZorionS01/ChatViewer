...
import com.szw.chatblog.dto.NoticeDto;
import com.szw.chatblog.dto.NoticeTypeDto;
import com.szw.chatblog.mapper.ArticleMapper;
import com.szw.chatblog.mapper.CommentMapper;
import com.szw.chatblog.mapper.NoticeMapper;
...
import com.szw.chatblog.constant.KafkaConstant;
import com.szw.chatblog.pojo.*;
...
public class NoticeServiceImpl extends ServiceImpl<NoticeMapper, Notice> implements NoticeService {
    ...
    public Event genLikeEvent(Integer entityType, Long entityId) {
        ...
        event.setTopic(KafkaConstant.TOPIC_LIKE);
        // 发送者为点赞的已登录用户
    ...
    public Event genCommentEvent(Comment comment) {
        Event event = new Event();
        event.setTopic(KafkaConstant.TOPIC_COMMENT);
    ...
    public Notice genLikeNotice(Long userId, Integer entityType, Long entityId) {
        ...
        notice.setNoticeType(KafkaConstant.NOTICE_LIKE_TYPE);
    ...
    public Notice genCommentNotice(Comment comment) {
        ...
        Notice notice = new Notice();
        notice.setNoticeType(KafkaConstant.NOTICE_COMMENT_TYPE);
    ...
    public NoticeDto toDto(Notice notice) {
        ...
        // 点赞通知 or 评论通知
        if (noticeType.equals(KafkaConstant.NOTICE_LIKE_TYPE)) {
            sBuilder.append("点赞了你");
        }
        else if (noticeType.equals(KafkaConstant.NOTICE_COMMENT_TYPE)) {
    ...
    public List<NoticeTypeDto> noticeBrief(Long userId) throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<Void> likeNoticeSearch = CompletableFuture.runAsync(() ->{
            ...
            brief.setNoticeType(KafkaConstant.NOTICE_LIKE_TYPE);
            brief.setLatestNotice(selectLatestNotice(userId, KafkaConstant.NOTICE_LIKE_TYPE));
            brief.setUnreadCount(selectUnreadCount(userId, KafkaConstant.NOTICE_LIKE_TYPE));
            // 如果该类别确实有消息
        ...
        CompletableFuture<Void> commentNoticeSearch = CompletableFuture.runAsync(() ->{
            NoticeTypeDto brief = new NoticeTypeDto();
            brief.setNoticeType(KafkaConstant.NOTICE_COMMENT_TYPE);
            brief.setLatestNotice(selectLatestNotice(userId, KafkaConstant.NOTICE_COMMENT_TYPE));
            brief.setUnreadCount(selectUnreadCount(userId, KafkaConstant.NOTICE_COMMENT_TYPE));
            if (brief.getLatestNotice() != null) {
                briefs.add(brief);
            }
        }, ThreadPoolUtil.pool);
