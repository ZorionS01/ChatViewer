...
import com.szw.chatblog.dto.CommentDto;
import com.szw.chatblog.event.EventProducer;
import com.szw.chatblog.mapper.CommentMapper;
import com.szw.chatblog.mapper.UserMapper;
import com.szw.chatblog.pojo.Comment;
...
import com.szw.chatblog.service.CommentService;
...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    private List<CommentDto> commentsOfWithReply(Long parentId, Integer commentType) throws ExecutionException, InterruptedException {
            ...
            if (commentType.equals(COMMENT_TYPE_ARTICLE)) {
                ...
                for (CommentDto commentDto : commentDtos) {
                    ...
                    try {
                        ...
                        log.error("ExecutionException occurred: " + e.getMessage(), e);
        ...
        task1.exceptionally(ex -> {
            if (ex.getCause() instanceof ExecutionException) {
                log.error("ExecutionException occurred in CompletableFuture: " + ex.getCause().getMessage(), ex.getCause());
            } else if (ex.getCause() instanceof InterruptedException) {
                log.error("InterruptedException occurred in CompletableFuture: " + ex.getCause().getMessage(), ex.getCause());
            } else {
                log.error("Other exception occurred in CompletableFuture: " + ex.getCause().getMessage(), ex.getCause());
            }
            return null; // 返回一个默认值或者null，表示异常已被处理
        }).join(); // 主线程等待任务完成
        ...
        CompletableFuture<Void> task2 = CompletableFuture.runAsync(() -> {
            for (CommentDto commentDto : commentDtos) {
                ...
                if (!likes.equals(LikeServiceImpl.ENTITY_NOT_EXIST_CODE)) {
    ...
    public List<Long> queryLikeComments(Long articleId, Long userId) {
        ...
        for (Long id : ids) {
            ...
            if (BooleanUtil.isTrue(stringRedisTemplate.hasKey(RedisConstant.redisEntityKey(1, id)))) {
                // (1)如果Redis中存在而MySQL中不存在，加入likeIds
                if (BooleanUtil.isTrue(stringRedisTemplate.opsForSet().isMember(RedisConstant.redisEntityKey(1, id), userId.toString()))
                        && !likeIds.contains(id)) {
                    likeIds.add(id);
                }
                // (2)如果Redis中不存在而MySQL中存在，从likeIds中删去
                else if (BooleanUtil.isFalse(stringRedisTemplate.opsForSet().isMember(RedisConstant.redisEntityKey(1, id), userId.toString()))) {
                    likeIds.remove(id);
                }
            }
        }
